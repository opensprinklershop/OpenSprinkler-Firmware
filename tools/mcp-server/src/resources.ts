...
export function registerResources(
  ...
  server.resource(
    "api-overview",
    "opensprinkler://api-overview",
    {
      description:
        "OpenSprinkler API reference – program encoding, sensor configuration, program adjustments, monitors, error codes",
  ...
  server.resource(
    ...
    async () => {
      try {
        const data = await getClient().get<Record<string, unknown>>("/ja");
        ...
        const lines: string[] = [
          ...
          `- **Hardware**: v${opts.hwv ?? "?"}  type=0x${((opts.hwt as number) ?? 0).toString(16).toUpperCase()}`,
          `- **Stations**: ${status.nstations ?? "?"}`,
          `- **Programs**: ${((data.programs as Record<string, unknown>)?.nprogs) ?? "?"}`,
          ...
          `- **Station Names**: ${JSON.stringify((stations as Record<string, unknown>).snames ?? [])}`,
          "",
