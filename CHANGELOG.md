...
### Added
- **ESP32-C5 Hardware-Selbsttest (Board-Bring-up/QC)**: Handbestückte ESP32-C5-Boards können jetzt einen zerstörungsfreien Hardware-Selbsttest ausführen. Er wird durch **gleichzeitiges Halten von B1+B2 beim Booten** gestartet und läuft **noch vor dem Mounten des externen Flash-Dateisystems** (direkt nach Display-/IO-Expander-Initialisierung) – so lässt sich selbst ein Board mit defektem Flash noch diagnostizieren; es erfolgt kein Dateisystem-Zugriff. Geprüft werden On-Board-Peripherie und GPIOs: I2C-Geräte (OLED, IO-Expander, optional RTC/EEPROM) werden geprobt, das IO-Expander-Konfigregister wird gelesen/zurückgeschrieben, das Schieberegister mit „alles AUS" getaktet (keine Station aktiv), Taster und Sensor-Eingänge auf Ruhepegel geprüft, freie GPIOs per interner Pull-ups/-downs auf Kurzschluss getestet und – anhand des physischen ESP32-C5-WROOM-1-Pad-Layouts – benachbarte Pads auf Lötbrücken (Kurzschluss zwischen Kontakten) geprüft (die USB-Serial-Leitungen IO11/IO12 sind ausgenommen). Das Ergebnis erscheint auf dem OLED (fehlerhafte Geräte/Pins, bei mehreren Seiten automatisch durchlaufend). Der Test ist speicherneutral (keine statischen Puffer) und **beendet sich immer über einen Reset** – ein beliebiger Tastendruck löst den Neustart aus.
